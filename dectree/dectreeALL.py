		...
		if(not flag):
			...
			for node in curr:
				try: #see if we are looking at a classification or feature
					...
					if str(node[0]) != "1.0" and str(node[0]) != "-1.0":
...
def split(seqs, node, dat, mots):
	...
	if(len(set(lactual)) == 1):
		numcimpneg = numcimppos = 0
		for x in range(1,len(dat)):
			if(dat[x][0] in lseqs):
				if(float(dat[x][len(dat[0])-1]) == 1.0):
					numcimppos += 1
				elif(float(dat[x][len(dat[0])-1]) == -1.0):
					numcimpneg += 1
		myTup = lactual[0],numcimppos,numcimpneg
		node.left.data = myTup
	else:
		findoptimal(lseqs,node.left,dat,mots)
		...
		for x in range(1,len(dat)):
			if(dat[x][0] in rseqs):
				if(float(dat[x][len(dat[0])-1]) == 1.0):
					...
					numcimpneg += 1
		myTup = ractual[0],numcimppos,numcimpneg
		node.right.data = myTup
	else:
		findoptimal(rseqs,node.right,dat,mots)

#findoptimal: uses phi to calculate the best motif to split at
#             input is the list of seqs
def findoptimal(seqs, node, dat, mots):
	numcols = len(dat[0])
	numrows = len(dat)
	motscores = {}
		...
		for x in range(1,len(dat)):
			if(dat[x][0] in seqs):
				if(float(dat[x][len(dat[0])-1]) == 1.0):
					...
					numcimpneg += 1
		myTup = (findMaj(seqs,dat),numcimppos,numcimpneg)
		node.data = myTup
		return
	else:
		node.data = selectedmotif
		split(seqs,node,dat,mots)

def decisionTree(listit):
	numrows = len(listit)
	...
	findoptimal(seqs,decTree,listit,mots)
	return decTree
#TESTING FNS BELOW
#------------------------------
		...
		for x in range(0, len(testdata)):
			if (testdata[x][0] == seq):
				try:
					...
					return str(model.data[0])


def runDecTreeOnTest(model, testdata):
	results = {}
	for x in range(1,len(testdata)):
		newval = treePredictSeq(testdata[x][0],model,testdata)
		if(newval == "1.0"): 
...
#train model
myDecTree = decisionTree(listit)

#testit
decResults = runDecTreeOnTest(myDecTree, listit)

evalResults(decResults,listit,1)
print("\n")
print("\n\n")
