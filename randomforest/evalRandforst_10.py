...
def printTree(root,flag=False,feats={}):
...
def evaluateForest(testingset,forest):
	...
	for x in range(1,len(testingset)):
		...
		answers[seq] = (statistics.mode(votes),forest[x])
	return answers

def evalResults(dictofseqs,givendat,store_tp_samp,store_fn_samp,i):
		...
		if(givendat[x][0] in dictofseqs):
			if(dictofseqs[givendat[x][0]][0] != float(givendat[x][len(givendat[0])-1])):
				...
				if(float(dictofseqs[givendat[x][0]][0]) == 1.0):
					...
					trueposfeats.write("---------------------------------------------------------\n")
					storedfeats = {}
					printTree(dictofseqs[givendat[x][0]][1],True,storedfeats) #get which features caused it
					for k, v in sorted(storedfeats.items(), key=lambda x:(-x[1],x[0])):
...
resultsfilepath = "../../" + datset + "_FEATUREINFO" + ".txt"
...
for k in range(10):
	...
	for x in range(0,numTrees): #this is number of trees and can be changed
		...
		#append class
		for x in range(0,len(traindat)):
			newset[x].append(traindat[x][len(traindat[0])-1])
	...
	evalResults(dictofseqs,testdat,store_tp_samp,store_fn_samp,k)
	print("\n")
