...
#end of yichaos definitions

#beginning of my definitions
...
for i in range(numfolds):
    ...
    for x in range(0,len(listit)):
        newrow = [listit[x][0]]
        for y in range(1,len(listit[x])):
            newrow.append(int(listit[x][y]))
        if(x in indices[i]):
            datatest.append(newrow)
            testcsv.append(listit[x])
        else:
            datatrain.append(newrow)
    trainset = pd.DataFrame(datatrain[1:],columns=header)
    trainset = trainset.set_index('')
    testset = pd.DataFrame(datatest[1:],columns=header)
    testset = testset.set_index('')
    greedy_result = open("Greedy."+str(k)+".result","wb")

    # preprocessing
    df = clean_data(trainset,k)
    Total_sets = trainset.columns[:-1]
    negative_data = trainset[trainset[trainset.columns[-1]] != 1][Total_sets]
    ...
    for c in trainset.columns[:-1]:
        covered_elements_dict[c] = positive_df[positive_df[c] >=1].index.tolist()

    # Greedy_PNPSKC
    selected_sets = LOW_DEG_RBSKC2(df,k)
    selected_sets_back = backward_elimination(trainset,k,selected_sets)

    # output result
    outresult = summerize_result(testset,k,selected_sets_back)
    model = selected_sets_back
    print(model)
    print("-----------------------------------")
    evalResults(runGreedyOnTest(model,testcsv),testcsv,0)
    print("-----------------------------------")
