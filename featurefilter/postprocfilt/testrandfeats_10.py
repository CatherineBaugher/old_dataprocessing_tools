...
numTrees = 100 #change value to change number of trees in the forest
class Tree(object):
	def __init__(self):
		self.left = None
		self.right = None
...
def evaluateForest(testingset,forest):
	...
	#Append only trees that voted positively
	answers = {}
	for x in range(1,len(testingset)):
		...
		for x in range(0,numTrees): #vote!
			prediction = treePredictSeq(seq,forest[x],testingset)
			if (prediction == 1.0):
				posforest.append(forest[x])
			votes.append(prediction)
		try:
			answers[seq] = (statistics.mode(votes),posforest)
		except: #tie-breaker: count it as a positive
			answers[seq] = (1.0,posforest)
	return answers

def evalResults(dictofseqs,givendat,store_tp_samp,store_fn_samp,storedfeats,numb):
	#NEW RULES FOR THIS RF VERSION ONLY:
	#Within store_tp_samp, only store once. then if it's a new TP sample, store the feats used
	#Within store_fn_samp, store false negative samples EVERY time they occur
		...
		if(givendat[x][0] in dictofseqs):
			if(dictofseqs[givendat[x][0]][0] != float(givendat[x][len(givendat[0])-1])):
				if(float(dictofseqs[givendat[x][0]][0]) == 1.0):
					...
					falseneg += 1
					if givendat[x][0] in store_fn_samp:
				...
				if(float(dictofseqs[givendat[x][0]][0]) == 1.0):
					truepos += 1
					if givendat[x][0] not in store_tp_samp: #want to get as many new TPs as possible!!
						store_tp_samp[givendat[x][0]] = 1 #mark sample as found.
						#note the feats of new sample to output later...
						...
						store_tp_samp[givendat[x][0]] += 1 #we can find out how many times it was correctly identified
				else:
					trueneg += 1
	tot = len(dictofseqs)
...
trueposfeats = open("./features_truepos","w") #this is a list to use in filtering
sampsfalseneg = open("./falsenegsamples.txt","w")


#RUN ITSELF MANY TIMES (100? 500?)
for u in range(0,500):
	...
	for numb in range(10):
		...
		evalResults(dictofseqs,testdat,store_tp_samp,store_fn_samp,storedfeats,numb)

		print("\n")



#at very end, print out all the info we gathered.
